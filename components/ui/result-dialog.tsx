...
export function ResultDialog({
  ...
  const getAgreementStatus = () => {
    if (!geminiResponse || !flaskResponse) return null;

    // For Kepler data
    if (geminiResponse.is_exoplanet !== undefined && flaskResponse.is_exoplanet !== undefined) {
      const agree = geminiResponse.is_exoplanet === flaskResponse.is_exoplanet;
      return {
        agree,
    ...
    if (geminiResponse.disposition && (flaskResponse.archive_disposition || flaskResponse.koi_pdisposition || flaskResponse.tfopwg_disp)) {
      const flaskDisposition = flaskResponse.archive_disposition || flaskResponse.koi_pdisposition || flaskResponse.tfopwg_disp || '';
      const agree = geminiResponse.disposition.toLowerCase() === flaskDisposition.toLowerCase();
      return {
        agree,
        icon: agree ? <CheckCircle2 className="h-4 w-4 text-green-400" /> : <AlertCircle className="h-4 w-4 text-yellow-400" />,
        text: agree ? "Models agree" : "Models disagree",
            ...
            {geminiResponse && (
              ...
              >
                ...
                <div className="flex items-center gap-2 pb-2 border-b border-gray-700">
                  ...
                  <h3 className="text-base font-bold text-blue-400">Gemini AI</h3>
                </div>
                ...
                <div className="space-y-2.5">
                  ...
                  <div>
                    ...
                    <Badge className={`${
                      geminiResponse.disposition === 'CONFIRMED' 
                        ? 'bg-green-500/20 text-green-300 border-green-400/30' 
                        : geminiResponse.disposition === 'FALSE POSITIVE'
                        ? 'bg-red-500/20 text-red-300 border-red-400/30'
                        : 'bg-yellow-500/20 text-yellow-300 border-yellow-400/30'
                    } border font-semibold text-xs`}>
                      {geminiResponse.disposition}
                    </Badge>
                  </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-xs font-bold text-blue-400">
                        {(geminiResponse.confidence * 100).toFixed(1)}%
                      </span>
                    </div>
                    <div className="bg-gray-800 rounded-full h-1.5 overflow-hidden">
                      <motion.div
                        initial={{ width: 0 }}
                        animate={{ width: `${geminiResponse.confidence * 100}%` }}
                  ...
                  {/* Is Exoplanet */}
                  {geminiResponse.is_exoplanet !== undefined && (
                    <div>
                      <p className="text-xs text-gray-500 mb-1">Classification</p>
                      <Badge variant="outline" className={`text-xs ${geminiResponse.is_exoplanet 
                        ? "border-green-400/50 text-green-300 bg-green-500/10" 
                        : "border-red-400/50 text-red-300 bg-red-500/10"}`}>
                        {geminiResponse.is_exoplanet ? '✓ Exoplanet' : '✗ Not Exoplanet'}
                      </Badge>
                    </div>
                  )}

                  {/* Habitability Assessment (K2 specific) */}
                  {geminiResponse.habitability_assessment && (
                    <div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="text-center">
                  ...
                  <p className="text-base font-bold text-purple-400">
                    {(((geminiResponse.confidence + flaskResponse.probability) / 2) * 100).toFixed(1)}%
                  </p>
                </div>
              </div>
            </motion.div>
